...
parser = argparse.ArgumentParser(description="Faithful CESN+ on Reach.")
...
class CESNPlus:
    def __init__(self, input_dim, action_dim, reservoir_size=1000,
                 spectral_radius=0.9, leaking_rate=0.3, input_scaling=1.0,
    ...
    def _run_reservoir(self, U):
        ...
        for t in range(T):
            u_t = self._normalize(U[t]).squeeze()
            pre = self.W_in @ torch.cat([ones, u_t]) + self.W @ x
    ...
    def _run_reservoir_batch(self, U_batch):
    ...
    def train(self, U, Y):
        self.u_mean = U.mean(dim=0, keepdim=True)
        self.u_std = U.std(dim=0, keepdim=True)
        X = self._run_reservoir(U)
        n, d = X.shape
        XtX = X.T @ X
        reg = self.ridge * torch.eye(d, device=DEVICE)
        self.XtX_inv = torch.linalg.inv(XtX + reg)
    ...
    def predict_batch(self, U_batch):
        X = self._run_reservoir_batch(U_batch)
...
def main():
    ...
    U = torch.tensor(data["obs"], dtype=torch.float32, device=DEVICE)
    Y = torch.tensor(data["action"], dtype=torch.float32, device=DEVICE)
    input_dim = U.shape[1]
    action_dim = Y.shape[1]
    print(f"[INFO] obs_dim={input_dim}, action_dim={action_dim}, samples={U.shape[0]}")
    ...
    start = time.time()
    X_train = model.train(U, Y)
    train_time = time.time() - start
    Y_hat = X_train @ model.W_out.T
    fit_err = torch.norm(Y_hat - Y, dim=1).mean().item()
    print("======================================")
    ...
    errors, pis = [], []
    print("[INFO] Evaluating CESN+ (recurrent reservoir)...")
    ...
    print("======================================")
    print(f"[RESULT] CESN+ Accuracy Evaluation (faithful impl.)")
